...
import { validateScenario } from './provider.js';
/**
 * AI 案例生成器
 */
export class AICaseGenerator {
     ...
     * 加载流程定义
     */
    loadProcessDefinition(configPath) {
     ...
     * 加载语义关键词
     */
    loadSemanticKeywords(keywordsPath) {
...
Output the scenarios as JSON.`;
     ...
     * 生成必要的基本场景
     */
    generateEssentialScenarios(options) {
        const scenarios = [];
 ...
 * 创建 AI 案例生成器
 */
export function createAICaseGenerator(provider) {
    return new AICaseGenerator(provider);
}
//# sourceMappingURL=case-generator.js.map