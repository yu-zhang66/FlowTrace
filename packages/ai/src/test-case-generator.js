 ...
 */
import { validateTestCase, AccountSwitchStepSchema, TestCaseStepSchema, PENDING_CONFIRMATION, ConfigCheckStatus } from '@flowtrace/core';
import { createWorkflowNodeExpander, createAccountSwitchGenerator } from '@flowtrace/collector';
// ============================================================
...
export class TestCaseGenerator {
    ...
    generateFullPathPassCase() {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            if (!isLast && i < steps.length - 1) {
                ...
                testCase.steps.push(...this.createAccountSwitchSteps(this.getAccountRef(step, i), this.getAccountRef(nextStep, i + 1), nextStep));
            }
        }
        // 设置预期结果
     ...
     * 生成拒绝类案例
     */
    generateRejectCase(rejectAtStep) {
        this.caseCounter++;
        const steps = this.getWorkflowSteps();
        ...
        for (let i = 0; i < rejectAtStep; i++) {
            ...
            if (i < rejectAtStep - 1) {
                const nextStep = steps[i + 1];
                testCase.steps.push(...this.createAccountSwitchSteps(this.getAccountRef(approvalStep, i), this.getAccountRef(nextStep, i + 1), nextStep));
            }
        }
        // 添加拒绝步骤
     ...
     * 生成退回类案例
     */
    generateReturnCase(returnAtStep) {
        this.caseCounter++;
        const steps = this.getWorkflowSteps();
     ...
     * 生成会签案例
     */
    generateCosignCase(cosignAtStep) {
        this.caseCounter++;
        const steps = this.getWorkflowSteps();
        ...
        for (let i = 0; i < cosignCount; i++) {
            ...
            if (i < cosignCount - 1) {
                ...
                testCase.steps.push(...this.createAccountSwitchSteps(currentSigner, nextSigner, step));
            }
        }
        // 设置预期结果
        testCase.expectedFlowState = {
     ...
     * 生成会签部分未完成案例
     */
    generateCosignPartialIncompleteCase() {
        this.caseCounter++;
        const steps = this.getWorkflowSteps();
    ...
    generateUnauthorizedCase() {
        ...
        testCase.steps.push(...this.createAccountSwitchSteps('initiator', unauthorizedAccount, this.getWorkflowSteps()[0]));
        // 添加无权限尝试步骤
    ...
    generateDuplicateApprovalCase() {
        ...
        // 模拟重复审批尝试（账号切换回原审批人）
        testCase.steps.push(...this.createAccountSwitchSteps(this.getAccountRef(step, stepIndex), this.getAccountRef(step, stepIndex), step));
        // 添加重复审批步骤
     ...
     * 获取节点的审批人
     */
    getApproversForNode(node) {
        if (node.actors && Array.isArray(node.actors)) {
            return node.actors.map((actor, index) => ({
                approverId: actor,
                approverCode: actor,
                approverName: actor,
                approverType: 'USER',
     ...
     * 创建审批步骤
     */
    createApprovalSteps(step, index, accountType) {
        const steps = [
            {
     ...
     * 创建转交流程
     */
    createTransferSteps(step, index) {
        const transferStep = {
     ...
     * 创建会签审批步骤
     */
    createCosignApprovalSteps(step, index, cosignIndex, totalCosigners) {
        const cosignStep = {
     ...
     * 创建会签拒绝步骤
     */
    createCosignRejectSteps(step, index, cosignIndex) {
        const cosignRejectStep = {
     ...
     * 创建无权限尝试步骤
     */
    createUnauthorizedAttemptStep(accountRef) {
     ...
     * 创建重复审批步骤
     */
    createDuplicateApprovalSteps(step, index) {
        const duplicateStep = {
     ...
     * 创建账号切换步骤
     */
    createAccountSwitchSteps(fromAccount, toAccount, targetStep) {
        const context = {
        ...
        const switchStep = this.accountSwitchGenerator.generateSingleSwitchStep(fromAccount, toAccount, context, {
     ...
     * 设置全路径通过的预期结果
     */
    setFullPathPassExpectations(testCase) {
        const steps = this.getWorkflowSteps();
 ...
 * 创建测试案例生成器
 */
export function createTestCaseGenerator(options) {
    return new TestCaseGenerator(options);
}
/**
 * 快速生成标准测试案例集合
 */
export function generateStandardTestCases(processId, flowchartDocument, configCheckResult, workflowConfig) {
