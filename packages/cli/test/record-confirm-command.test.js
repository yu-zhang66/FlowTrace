...
function initProject(projectRoot, opts = {}) {
    ...
    if (opts.processIds) {
        ...
        for (const pid of opts.processIds) {
            writeFileSync(join(processesDir, `${pid}.json`), JSON.stringify({
                processId: pid,
...
describe('record-confirm command', () => {
    ...
    describe('when multiple processes exist and no process is specified', () => {
        it('returns AMBIGUOUS_PROCESS with candidates and exit code 2', async () => {
            ...
            const processesDir = join(flowtraceRoot, 'processes');
            writeFileSync(join(processesDir, 'login.json'), JSON.stringify({ processId: 'login', name: 'Login', keywords: ['login'] }));
            writeFileSync(join(processesDir, 'logout.json'), JSON.stringify({ processId: 'logout', name: 'Logout', keywords: ['logout', 'login'] }));
            writeFileSync(join(processesDir, 'submit-order.json'), JSON.stringify({ processId: 'submit-order', name: 'Submit Order', keywords: ['login'] }));
            const captured = captureExit();
            try {
                await recordConfirmCommand({ project: projectRoot });
            }
            ...
            expect(lastRecordConfirmResult.remediation.some((r) => r.includes('Did you mean'))).toBe(true);
        });
    });
    // -------------------------------------------------------------------------
    // PROCESS_NOT_FOUND
...
//# sourceMappingURL=record-confirm-command.test.js.map