...
describe('Project Isolation Tests', () => {
  ...
  describe('CLI loads scenarios from target project only', () => {
    beforeEach(() => {
      ...
      writeFileSync(
        join(projectA, '.flowtrace', 'flowtrace.yaml'),
        yaml.dump(flowtraceYaml)
      );
      ...
      writeFileSync(
        join(projectA, '.flowtrace', 'scenarios', 'login.yaml'),
        yaml.dump(loginScenarioA)
      );
      ...
      writeFileSync(
        join(projectB, '.flowtrace', 'flowtrace.yaml'),
        yaml.dump(flowtraceYamlB)
      );
      ...
      writeFileSync(
        join(projectB, '.flowtrace', 'scenarios', 'login.yaml'),
        yaml.dump(loginScenarioB)
      );
    });
  ...
  describe('CLI reports and executions go to target project', () => {
    beforeEach(() => {
      ...
      writeFileSync(
        join(projectA, '.flowtrace', 'scenarios', 'login.yaml'),
        yaml.dump(loginScenario)
      );
    });

    it('should use target project paths from config', async () => {
      const { loadTargetProjectConfig, getExecutionsDir, getReportsDir, getScenariosDir } = 
        await import('@flowtrace/core');
