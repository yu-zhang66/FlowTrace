...
function initProject(projectRoot, opts = {}) {
    ...
    if (opts.processIds) {
        ...
        for (const pid of opts.processIds) {
            writeFileSync(join(processesDir, `${pid}.json`), JSON.stringify({ processId: pid, name: pid.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) }));
        }
    }
...
describe('status command', () => {
    ...
    describe('process resolution', () => {
        it('resolves a process by query with exactly one match', async () => {
            ...
            const processesDir = join(flowtraceRoot, 'processes');
            writeFileSync(join(processesDir, 'submit-order.json'), JSON.stringify({ processId: 'submit-order', name: 'Submit Order', keywords: ['submit', 'order', '订单提交'] }));
            const captured = captureExit();
            try {
        ...
        it('returns AMBIGUOUS_PROCESS with candidates when query matches multiple processes', async () => {
            ...
            const processesDir = join(flowtraceRoot, 'processes');
            writeFileSync(join(processesDir, 'login.json'), JSON.stringify({ processId: 'login', name: 'Login', keywords: ['login', 'signin', '登录'] }));
            writeFileSync(join(processesDir, 'logout.json'), JSON.stringify({ processId: 'logout', name: 'Logout', keywords: ['logout', 'signout', '登录'] }));
            const captured = captureExit();
            try {
            ...
            expect(lastStatusResult?.remediation.some((r) => r.includes('Did you mean'))).toBe(true);
        });
    ...
    describe('exit code mapping', () => {
        it('exits 0 for READY status', async () => {
            ...
            writeFileSync(join(scenariosDir, 'login-success-001.yaml'), yaml.dump({
...
//# sourceMappingURL=status-command.test.js.map