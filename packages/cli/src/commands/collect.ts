...
import { generateId, loadTargetProjectConfig, validateTargetConfig, getFactsDir, runGate, gateForCommand, type CommandResult } from '@flowtrace/core';
import type { Collector, CollectorContext, CollectedFact } from '@flowtrace/core';
import type { Fact, FactCategory } from '@flowtrace/core';
import {
  createDemoCollector,
  createSourceCollector,
  createSourceCollectorConfig,
  createConfigCollectorLoader,
  executeCollection
...
export async function collectCommand(options: CollectOptions): Promise<void> {
  ...
  try {
    ...
    targetConfig = {
      ...
      database: { type: 'oracle' as const, configSource: '', access: 'read-only-collection' as const },
      ...
      execution: {
        mode: 'dual-run' as const,
        allowOnlineWrite: false,
        databaseMode: 'snapshot-only' as const,
        testDataMode: 'masked-or-snapshot' as const,
        failOn: ['P0', 'P1']
      }
    };
  }
  ...
  if (options.demo) {
    ...
    if (options.collector === 'demo') {
      ...
      const sourceCollector = createSourceCollector(
        createSourceCollectorConfig('source-collector', { sourceRoot })
      );
  ...
  if (collectors.length === 0 && !options.demo) {
    // Config-driven projects may already have a confirmed declarative process
    // while their source language is not supported by the Java source scanner
    // (for example a TypeScript/Express mock). Collect the process DSL itself
    // as evidence instead of incorrectly requiring the demo collector.
    const configuredCollector = createConfiguredProcessCollector(flowtraceRoot, processId);
    if (configuredCollector) {
      await configuredCollector.initialize(context);
      collectors.push(configuredCollector);
      console.log(chalk.gray(`     ✓ Configured process collector initialized`));
    }
  }
  ...
  for (const fact of facts) {
    ...
    if (fact.reviewStatus === 'AUTO_EXTRACTED') autoExtracted++;
    else if (fact.reviewStatus === 'PENDING_CONFIRM') pendingConfirm++;
    else if (fact.reviewStatus === 'CONFIRMED') confirmed++;
  }
  ...
  if (warnings.length > 0) {
    ...
    warnings.slice(0, 5).forEach((w: string) => console.log(chalk.gray(`   - ${w}`)));
  }

  if (errors.length > 0) {
    console.log(chalk.red(`\n✗ Errors (${errors.length}):`));
    errors.slice(0, 5).forEach((e: string) => console.log(chalk.gray(`   - ${e}`)));
  }

  const pendingCount = facts.filter((f: CollectedFact) => 
    f.reviewStatus === 'AUTO_EXTRACTED' || f.reviewStatus === 'PENDING_CONFIRM'
  ).length;
  if (pendingCount > 0) {
...
function createConfiguredProcessCollector(flowtraceRoot: string, processId: string): Collector | null {
  const processPath = resolve(flowtraceRoot, 'processes', `${processId}.yaml`);
  if (!existsSync(processPath)) return null;
  let processDefinition: any;
  try {
    processDefinition = yaml.load(readFileSync(processPath, 'utf-8')) as any;
  } catch {
    return null;
  }
  if (!processDefinition || typeof processDefinition !== 'object') return null;

  const now = new Date().toISOString();
  const evidence = [{ source: processPath, confidence: 1, extractedAt: now }];
  const fact = (type: string, category: string, name: string, content: Record<string, any>): CollectedFact => ({
    id: generateId('fact'),
    type,
    category,
    name,
    description: `Declarative process asset collected from ${processPath}`,
    content,
    evidence,
    reviewStatus: 'CONFIRMED',
    collectorType: 'config-scanner',
    collectorName: 'configured-process-collector',
    confidence: 1,
    collectedAt: now,
  });

  const collector: Collector = {
    name: 'configured-process-collector',
    type: 'config-scanner',
    config: { name: 'configured-process-collector', type: 'config-scanner', enabled: true },
    async initialize() {},
    async checkAvailability() { return { available: true }; },
    async cleanup() {},
    async collect() {
      const facts: CollectedFact[] = [fact('process-definition', 'process_definition', String(processDefinition.name ?? processId), processDefinition)];
      for (const action of Array.isArray(processDefinition.actions) ? processDefinition.actions : []) {
        facts.push(fact('business-action', 'action', String(action.id ?? action.type ?? 'action'), action));
      }
      for (const transition of Array.isArray(processDefinition.fsm?.transitions) ? processDefinition.fsm.transitions : []) {
        facts.push(fact('transition-rule', 'rule', `${transition.from ?? '?'} -> ${transition.to ?? '?'}`, transition));
      }
      return facts;
    },
  };
...
function generateProcessArtifacts(targetConfig: any, projectPath: string, processId: string): { jsonFile: string; mdFile: string } {
  ...
  let process: any = { processId, name: targetConfig.project.name, nodes: [], transitions: [] };
  if (existsSync(processFile)) process = yaml.load(readFileSync(processFile, 'utf-8')) as any;
  if (!Array.isArray(process.nodes) || process.nodes.length === 0) {
    const actions = Array.isArray(process.actions) ? process.actions : [];
    const actionNodes = actions.map((action: any, index: number) => ({
      id: String(action.id ?? action.type ?? `action-${index + 1}`),
      type: 'task',
      name: String(action.label ?? action.id ?? action.type ?? `Action ${index + 1}`),
      actor: action.actor,
    }));
    const fsmTransitions = Array.isArray(process.fsm?.transitions) ? process.fsm.transitions : [];
    process.nodes = actionNodes.length > 0
      ? [{ id: 'start', type: 'start', name: '开始' }, ...actionNodes, { id: 'end', type: 'end', name: '结束' }]
      : [];
    process.transitions = fsmTransitions.length > 0
      ? fsmTransitions.map((transition: any) => ({
        from: String(transition.from),
        to: String(transition.to),
        event: transition.action,
      }))
      : actionNodes.slice(0, -1).map((node: any, index: number) => ({ from: node.id, to: actionNodes[index + 1]?.id }));
  }
  if (process.nodes.length === 0 && existsSync(inventoryFile)) {
    const inventory = JSON.parse(readFileSync(inventoryFile, 'utf-8'));
    const discovered = (inventory.processes || []).find((item: any) => item.processId === processId);
    if (discovered) process = discovered;
  }
  const jsonFile = join(processDir, `${processId}.json`);
  writeFileSync(jsonFile, JSON.stringify(process, null, 2), 'utf-8');
  const nodeNames = new Map((process.nodes || []).map((n: any) => [n.id, n.name || n.id]));
  const lines = ['# 流程采集结果（人工流程图）', '', '```mermaid', 'flowchart LR'];
  const mermaidId = (value: unknown, index: number) => {
    const normalized = String(value || `node_${index}`).replace(/[^a-zA-Z0-9_]/g, '_');
    if (normalized.toLowerCase() === 'end') return 'flow_end';
    return /^[a-zA-Z_]/.test(normalized) ? normalized : `node_${normalized}`;
  };
  const ids = new Map((process.nodes || []).map((n: any, i: number) => [String(n.id), mermaidId(n.id, i)]));
  ...
  for (const [index, node] of (process.nodes || []).entries()) lines.push(`| ${index + 1} | ${node.name || node.id} | ${node.actor || '-'} |`);
 ...
 */
function convertToFact(collected: CollectedFact): Fact {
  return {
    id: collected.id,
    type: collected.type,
    category: collected.category as FactCategory,
    name: collected.name,
 ...
 * 生成 Markdown 报告
 */
function generateBaselineMarkdown(
  baseline: any, 
