...
import {
  ...
  resolveScenarios,
  type ScenarioResolutionResult
} from './scenario-resolver.js';
import {
  createLoginTestExecutor,
  LoginTestExecutor
...
interface LegacyTestConfig {
  ...
  captcha?: {
    ...
    strategy?: 'paddleocr' | 'test-mode' | 'disabled';
    maxRetries?: number;
  };
}
...
interface LoginTestTargetConfig {
  legacy: LegacyTestConfig;
  current: CurrentTestConfig;
}
...
function initializeExecutionContext(
  ...
  const mode: 'single-browser' | 'dual-browser' = options.mode === 'dual-browser' ? 'dual-browser' : 'single-browser';
 ...
 */
async function stepCollectBaseline(ctx: ExecutionContext): Promise<{ success: boolean; error?: string }> {
 ...
 */
async function stepGenerateCases(ctx: ExecutionContext, options: TestCommandOptions): Promise<{
  success: boolean;
  scenarios: Scenario[];
  resolutionResult: ScenarioResolutionResult;
  error?: string;
}> {
  ...
  let resolutionResult: ScenarioResolutionResult;
  ...
  if (resolutionResult.invalidScenarios.length > 0) {
    ...
    for (const inv of resolutionResult.invalidScenarios) {
      console.log(chalk.gray(`    - ${(inv.scenario as any)?.id || 'unknown'}: ${inv.errors.join(', ')}`));
    }
  }
 ...
 */
async function stepValidateCases(
  ...
  if (resolutionResult.invalidScenarios.length > 0) {
    for (const inv of resolutionResult.invalidScenarios) {
      errors.push(`Invalid scenario ${(inv.scenario as any)?.id}: ${inv.errors.join(', ')}`);
    }
  }
 ...
 */
async function loadHistoricalScenarios(ctx: ExecutionContext): Promise<Scenario[]> {
 ...
 */
async function stepLoadAdapters(
  ctx: ExecutionContext,
  loginConfig: LoginTestTargetConfig
): Promise<{ success: boolean; adapters: any; error?: string }> {
 ...
 */
async function stepExecuteScenarios(
  ctx: ExecutionContext,
  scenarios: Scenario[],
  adapters: any,
  targetConfig: any,
 ...
 */
async function stepCollectEvidence(
  ctx: ExecutionContext,
  run: any
): Promise<{ success: boolean; hasScreenshots: boolean; error?: string }> {
  ...
  const casesWithEvidence = caseResults.filter((c: any) =>
    (c.legacyResult?.finalObservation?.evidence?.length > 0) ||
 ...
 */
async function stepGenerateReports(
  ctx: ExecutionContext,
  run: any
): Promise<{ success: boolean; reportPaths?: any; error?: string }> {
 ...
 */
async function runUnifiedTestFlow(
  options: TestCommandOptions,
  projectPath: string,
  targetConfig: any,
  loginConfig: LoginTestTargetConfig
): Promise<TestFlowResult> {
  const flowSteps: TestFlowStep[] = [
  ...
  let resolutionResult: ScenarioResolutionResult = { validScenarios: [], invalidScenarios: [], parseErrors: [] };
...
function loadLoginTestConfig(
  projectPath: string,
  options: TestCommandOptions,
  targetConfig: any
): LoginTestTargetConfig | null {
  const configuredPath = targetConfig?.loginTest?.configFile as string | undefined;
  ...
  for (const configPath of candidatePaths) {
    if (existsSync(configPath)) {
      try {
        ...
        return config as LoginTestTargetConfig;
      } catch (error) {
...
function checkConfigurationExists(projectPath: string): string[] {
  const issues: string[] = [];
...
function printInitHint(projectPath: string): void {
  console.log(chalk.yellow(`\nTo initialize FlowTrace for this project, run:`));
  console.log(chalk.cyan(`   flowtrace init --project ${projectPath}`));
}

async function loadAdapters(
  config: LoginTestTargetConfig,
  projectPath: string,
  isLegacyShadow: boolean,
  evidenceDir?: string
): Promise<{ legacyAdapter: any; currentAdapter: any; cleanup: () => Promise<void> }> {
  const { LoginAdapterLoader } = await import('@flowtrace/adapter');
  
  const legacyCfg = (config.legacy || {}) as Record<string, any>;
  ...
  const adapterConfig = {
    ...
    evidence: {
      ...
      screenshot: (legacyCfg.screenshotStrategy as 'always' | 'on-failure' | 'never') || 'always' as const,
...
function generateMarkdownReport(run: any, projectPath: string): string {
  const lines: string[] = [];
  ...
  for (const caseResult of run.caseResults) {
    ...
    ].filter((item: any) => item.type === 'screenshot');
...
export async function saveReport(run: any, projectPath: string, executionsDir?: string): Promise<{ jsonPath: string; mdPath: string; htmlPath: string }> {
  ...
  const isSuccessState = (state: string) => financingSuccessStates.includes(state);

  const caseSectionsHtml = run.caseResults.map((item: any) => {
    ...
    const renderObs = (label: string, badgeClass: string, obs: any, ev: any[]) => {
      if (!obs) return '';
...
export async function testCommand(options: TestCommandOptions): Promise<void> {
  ...
  for (const target of [loginConfig.legacy, loginConfig.current]) {
    if (target?.username && target.usernameEnv && !process.env[target.usernameEnv]) process.env[target.usernameEnv] = target.username;
    if (target?.password && target.passwordEnv && !process.env[target.passwordEnv]) process.env[target.passwordEnv] = target.password;
  }
  for (const [key, value] of Object.entries((loginConfig as any).credentials || {})) {
