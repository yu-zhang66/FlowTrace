...
function runCodegen(args: string[], cwd: string): Promise<number> {
...
export function resolveRecordingPath(recordingsDir: string, requested: string | undefined, fallbackName: string): string {
...
export async function recordCommand(options: RecordCommandOptions): Promise<void> {
