 ...
 */
function collectEvidence(projectPath: string): string {
  ...
  const searchInDir = (dir: string, depth: number = 0) => {
 ...
 */
function discoverCandidatesFromEvidence(evidence: string): any[] {
  const result: any[] = [];
...
export async function discoverProcessesCommand(options: DiscoverOptions): Promise<void> {
  ...
  if (options.ai || process.env.FLOWTRACE_AI_BASE_URL || deepseekKey) {
    try {
      ...
      processes = await provider.completeJSON<any[]>({
...
function renderInventory(inventory: any): string {
  const lines = [
...
function renderProcess(process: any): string {
  const lines = [
    `# ${process.name}`,
    ``,
