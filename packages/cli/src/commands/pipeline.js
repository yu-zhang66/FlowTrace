...
import { generateId, loadTargetProjectConfig, runGate, gateForCommand, ensureReportCompleteness, createErrorVerificationRun } from '@flowtrace/core';
/**
...
export async function pipelineCommand(options) {
    ...
    const artifacts = [];
    let finalError = null;
    let currentStage = 'collect';
    let runId = null;
    // Persist helper
    const persistState = (stageName, status, message) => {
 ...
 */
async function executeCollectStage(context, state, artifacts) {
 ...
 */
async function executeGenerateCasesStage(context, state, artifacts) {
 ...
 */
async function executeVerifyStage(context, state, artifacts) {
 ...
 */
async function executeReportStage(context, state, artifacts, runId) {
 ...
 */
async function generateReportsOnFailure(context, state, artifacts, error) {
...
function loadResumeState(flowtraceRoot) {
...
function saveResumeState(flowtraceRoot, state) {
    const path = join(flowtraceRoot, 'pipeline-resume.json');
    writeFileSync(path, JSON.stringify(state, null, 2), 'utf8');
}
// Artifact helpers
function createArtifact(label, flowtraceRoot, relativePath) {
    const absolutePath = join(flowtraceRoot, relativePath);
    const ext = relativePath.split('.').pop()?.toLowerCase() || 'json';
    const type = ext === 'json' ? 'json' : ext === 'md' ? 'markdown' : ext === 'html' ? 'html' : 'json';
    return {
        label,
        path: relativePath,
        absolutePath,
...
function collectEvidence(projectRoot, config, processId, taskId) {
    const now = new Date().toISOString();
    const sourceRoot = config.project.sourceRoot || projectRoot;
    const sources = [];
    const add = (type, path, summary, data) => sources.push({
