...
function checkRenderDrift(loader, config, type) {
    let hasDrift = false;
    if (type === 'all' || type === 'baseline') {
...
function renderBaseline(baselineDir) {
    ...
    try {
        ...
        markdown += `\`\`\`mermaid\n`;
        markdown += `graph TD\n`;
        const processFact = baseline.facts.find((f) => f.category === 'process_definition');
        ...
        const rules = baseline.facts.filter((f) => f.category === 'rule');
        ...
        markdown += `## Roles\n\n`;
        const roles = baseline.facts.filter((f) => f.category === 'role');
        ...
        const dataEffects = baseline.facts.filter((f) => f.category === 'data_effect');
        ...
        const externalCalls = baseline.facts.filter((f) => f.category === 'external_call');
...
function renderScenarios(scenariosDir) {
    ...
    try {
        ...
        const scenarioFiles = files.filter((f) => f.endsWith('.yaml') || f.endsWith('.yml') || f.endsWith('.json'));
...
function renderReports(reportsDir) {
    ...
    try {
        ...
        const reportFiles = files.filter((f) => f.endsWith('.json') && f.startsWith('run-'));
