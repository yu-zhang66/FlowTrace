...
import { loadTargetProjectConfig, getFactsDir, getScenariosDir, getSemanticDir, runGate, gateForCommand } from '@flowtrace/core';
import { AICaseGenerator } from '@flowtrace/ai';
export async function generateCasesCommand(options) {
    ...
    const scenariosJson = {
        ...
        counts: {
            ...
            enabledCount: merged.filter((s) => s.enabled !== false).length
        }
    };
...
function loadBaseline(factsDir) {
...
function loadSemanticKeywords(semanticDir) {
...
function loadProcessDefinition(semanticDir, processId) {
    if (processId) {
        const discoveredPath = resolve(semanticDir, '..', 'processes', `${processId}.json`);
        if (existsSync(discoveredPath)) {
            try {
                return JSON.parse(readFileSync(discoveredPath, 'utf-8'));
            }
            catch { /* fall through */ }
        }
    }
...
function loadExistingScenarios(outputDir) {
...
function mergeScenarios(existing, generated) {
    const seen = new Set();
    const merged = [];
    const fingerprint = (scenario) => JSON.stringify({
        name: scenario.name,
 ...
 * 确定性场景生成（AI 不可用时的回退）
 */
function generateDeterministicScenarios(processId, baseline, keywords) {
    const scenarios = [];
    const warnings = [];
    const errors = [];
    const nextId = (id) => `${processId}-${id}`;
    // 1. 正常审批
    scenarios.push({
        id: nextId('normal'),
        name: '正常审批流程',
...
function generateScenariosMarkdown(scenarios, processId, factCount, result) {
    let md = `# 测试场景
 ...
 */
function generateHumanReadableScenarios(scenarios, processId) {
    let md = `# 测试案例 - 人工展示
...
function categorizeScenario(scenario) {
    ...
    const actions = scenario.actions?.map((a) => a.type) || [];
...
//# sourceMappingURL=generate-cases.js.map