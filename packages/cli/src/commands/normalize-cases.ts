...
export interface NormalizeCasesOptions { project?: string; process?: string; input?: string; output?: string; }

export function normalizeCasesCommand(options: NormalizeCasesOptions): void {
  ...
  if (!inputs.length) throw new Error(`No raw recordings found under ${recordingDir}`);
  const outputDir = resolve(config.flowtraceRoot, 'scenarios'); mkdirSync(outputDir, { recursive: true });
  ...
  if (!output.startsWith(outputDir + '/') && output !== outputDir) throw new Error('Normalized scenarios must remain under .flowtrace/scenarios');
