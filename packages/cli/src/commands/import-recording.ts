...
import { loadTargetProjectConfig, type RecordingArtifact, type RecordedStep } from '@flowtrace/core';
...
type MappingFile = {
  mappings?: Array<{ locator: string; semanticAction: string }>;
};

function locatorKey(step: RecordedStep): string | undefined {
  if (step.type !== 'click' && step.type !== 'fill' && step.type !== 'assertion') return undefined;
  return step.locator;
}

function loadMappings(projectRoot: string, path?: string): Map<string, string> {
  ...
  const parsed = yaml.load(readFileSync(mappingPath, 'utf8')) as MappingFile | undefined;
  ...
  return new Map(entries.filter(entry => entry?.locator && entry?.semanticAction)
    .map(entry => [entry.locator, entry.semanticAction] as const));
}

function actionFromStep(step: RecordedStep, mappings: Map<string, string>): string | undefined {
  const key = locatorKey(step);
  return key ? mappings.get(key) : undefined;
}

function scenarioIdFromInput(input: string): string {
  return basename(input).replace(/\.(spec|test)\.[^.]+$/, '').replace(/[^a-zA-Z0-9_-]+/g, '-');
}

function inferFinalState(assertions: Array<Record<string, unknown>>): string {
  const stateAssertion = [...assertions].reverse().find(assertion =>
    assertion.assertionType === 'text' &&
...
): { scenario: Record<string, unknown>; unmapped: string[] } {
...
export async function importRecordingCommand(options: ImportRecordingOptions): Promise<void> {
  ...
  if (!processId) throw new Error('A process is required: pass --process <id>.');
  ...
  if (unmapped.length > 0) {
    ...
    for (const item of unmapped) console.log(chalk.gray(`    - ${item}`));
  }
}
