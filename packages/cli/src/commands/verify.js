...
import { loadTargetProjectConfig, validateTargetConfig, getScenariosDir, getReportsDir, getSemanticDir, validateAgainstSemanticModel } from '@flowtrace/core';
...
function projectUsesBuiltinRuntime(projectPath) {
    ...
    if (!existsSync(configPath))
        return false;
    try {
        const raw = yaml.load(fsReadFileSync(configPath, 'utf8'));
...
export async function verifyCommand(options) {
    ...
    // 加载场景
    const scenarios = loadScenariosFromDir(scenariosDir).filter((s) => s.enabled !== false);
    ...
    }, legacyContext, projectPath, resolve(projectPath, '.flowtrace'), {
    ...
    if (!loadedAdapters.legacy) {
        ...
        loadedAdapters.errors.forEach((e) => console.log(chalk.gray(`   - ${e}`)));
    ...
    try {
        ...
        for (const scenario of scenarios) {
            ...
            const dualResult = await executeScenarioWithAdapters(scenario, legacyAdapter, currentAdapter, semanticModel);
            results.push({
        ...
        const blockedBy = Object.entries(differencesBySeverity)
            .filter(([severity, count]) => count > 0 && targetConfig.execution.failOn.includes(severity))
            .map(([severity, count]) => `${severity}: ${count} difference(s)`);
        if (executionFailures > 0)
            blockedBy.push(`EXECUTION: ${executionFailures} scenario(s) failed to execute`);
...
async function resetAdapterState(adapter) {
 ...
 * 从目录加载场景
 */
function loadScenariosFromDir(scenariosDir) {
    const scenarios = [];
    // 尝试加载 scenarios.json
...
async function executeScenarioWithAdapters(scenario, legacyAdapter, currentAdapter, semanticModel) {
    let legacyResult;
    let currentResult;
    let error;
    try {
    ...
    if (legacyResult && currentResult) {
        ...
        if (legacyResult.businessData && currentResult.businessData) {
            ...
            const diffKeys = commonKeys.filter(k => JSON.stringify(legacyResult.businessData[k]) !== JSON.stringify(currentResult.businessData[k]));
        ...
        if (legacyResult.actions) {
            for (const a of legacyResult.actions)
                if (a.actor)
                    legacyRoles.add(a.actor);
        }
        if (currentResult.actions) {
            for (const a of currentResult.actions)
                if (a.actor)
                    currentRoles.add(a.actor);
        }
    ...
    const legacyActionError = legacyResult?.error || legacyResult?.metadata?.actionError;
    const currentActionError = currentResult?.error || currentResult?.metadata?.actionError;
...
async function executeScenarioCompat(adapter, scenario) {
    if (typeof adapter.executeScenario === 'function')
        return adapter.executeScenario(scenario);
    const actions = [];
    let last = {};
    let error;
    ...
    const queried = typeof adapter.queryResult === 'function' ? await adapter.queryResult() : {};
    return {
        scenarioId: scenario.id,
...
function normalizeSemanticEvent(value) {
    if (value === 'SUBMIT' || value.includes('SUBMITTED'))
        return 'SUBMIT';
    if (value.includes('CORE_APPROVED'))
        return 'CORE_APPROVAL';
    if (value.includes('RISK_ASSESSED'))
        return 'RISK_ASSESSMENT';
    if (value.includes('FINANCE_APPROVED'))
        return 'FINANCE_APPROVAL';
    if (value.includes('COUNTERSIGN'))
        return 'COUNTERSIGN';
 ...
 * 加载语义模型
 */
function loadSemanticModel(semanticDir, processId) {
 ...
 * 生成执行过程的 Markdown 报告
 */
function generateExecutionMarkdownReport(executionsDir, run, config, isLegacyShadow) {
    let md = `# 执行过程报告
...
`;
    for (const result of run.scenarios) {
        ...
        if (result.legacyResult?.actions && result.legacyResult.actions.length > 0) {
            ...
            md += `| 序号 | 动作 | 执行者 | 状态 |\n`;
            md += `|------|------|--------|------|\n`;
            result.legacyResult.actions.forEach((action, idx) => {
        ...
        if (result.currentResult?.actions && result.currentResult.actions.length > 0) {
            ...
            md += `| 序号 | 动作 | 执行者 | 状态 |\n`;
            md += `|------|------|--------|------|\n`;
            result.currentResult.actions.forEach((action, idx) => {
                const actionStatus = action.error ? '❌' : '✅';
                md += `| ${idx + 1} | ${action.type} | ${action.actor || '-'} | ${actionStatus} |\n`;
            });
            md += `\n`;
...
function detectExecutionSeverity(differences) {
