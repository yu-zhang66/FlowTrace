...
import { loadTargetProjectConfig, resolveProcess, computeStatus, statusFromError, } from '@flowtrace/core';
/** Set by statusCommand; available for programmatic callers after the command runs. */
export let lastStatusResult;
function printHumanStatus(snapshot) {
    ...
    if (snapshot.recording) {
        const recStatusColor = snapshot.recording.status === 'CONFIRMED'
            ? chalk.green
            : snapshot.recording.status === 'RECORDED'
                ? chalk.yellow
                : chalk.gray;
        console.log(chalk.gray(`Recording: ${recStatusColor(snapshot.recording.status)}`));
        if (snapshot.recording.processId)
            console.log(chalk.gray(`  Process : ${snapshot.recording.processId}`));
        if (snapshot.recording.artifact)
            console.log(chalk.gray(`  Artifact: ${snapshot.recording.artifact}`));
        if (snapshot.recording.confirmedBy)
            console.log(chalk.gray(`  By      : ${snapshot.recording.confirmedBy}`));
    }
    else {
    ...
    if (snapshot.run) {
        if (snapshot.run.runId)
            console.log(chalk.gray(`Run ID   : ${snapshot.run.runId}`));
        if (snapshot.run.currentStage)
            console.log(chalk.gray(`Stage    : ${snapshot.run.currentStage}`));
    }
    console.log();
}
function buildNotInitializedResult(projectRoot) {
...
export async function statusCommand(options) {
    ...
    try {
        ...
        const now = new Date().toISOString();
        const snapshot = statusFromError(projectRoot, 'CONFIG_INCOMPLETE', err instanceof Error ? err.message : String(err));
        lastStatusResult = {
            ok: false,
...
function snapshotToCommandCode(status) {
    switch (status) {
        case 'READY':
            return 'OK';
