...
export interface DualRunOptions { project?: string; scenario: string; legacyShadow?: boolean; }

export async function dualRunCommand(options: DualRunOptions): Promise<void> {
  ...
  const output = join(evidenceDir, 'dual-run.json'); writeFileSync(output, JSON.stringify(result, null, 2), 'utf8');
