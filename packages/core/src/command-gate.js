...
export function gateForCommand(command) {
    switch (command) {
        case 'collect':
            return ['init', 'config'];
        case 'generate-cases':
...
function evaluateRequirement(requirement, snapshot, processResolution, projectRoot) {
    switch (requirement) {
        case 'init':
...
function resolvedByFor(resolution, process) {
