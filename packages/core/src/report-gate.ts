...
import type { VerificationRun } from './models/report.js';
import type { ReleaseGate } from './models/command-result.js';
import { generateId } from './config.js';
import { getReportsDir } from './target-config.js';
...
export interface EnsureReportCompletenessOptions {
  runId: string;
  projectRoot: string;
 ...
 */
export function ensureReportCompleteness(
  options: EnsureReportCompletenessOptions
  ...
  try {
    ...
    run = JSON.parse(content) as VerificationRun;
  } catch (error) {
 ...
 */
function findRunFile(reportsDir: string, runId: string): string | null {
  const candidates = [
 ...
 */
function getReportsDirFromPath(flowtraceRoot: string): string {
 ...
 */
function generateJsonReport(run: VerificationRun): string {
 ...
 */
function generateMarkdownReport(run: VerificationRun, projectName: string): string {
 ...
 */
function generateHtmlReport(run: VerificationRun, projectName: string): string {
  const passRate = run.summary.total > 0
    ? ((run.summary.passed / run.summary.total) * 100).toFixed(1)
    : '0';
...
function screenshotList(result: any): any[] {
  ...
  return source.flatMap((item: any) => {
    if (typeof item === 'string') return [{ path: item, label: '' }];
...
function renderSystemHtml(result: any, label: string): string {
  if (!result) return `<div class="system"><h4>${label}</h4><p>未执行</p></div>`;
  const shots = screenshotList(result);
  const images = shots.length ? `<div class="shots">${shots.map((shot: any) => `<figure class="shot"><a href="${shot.path}" target="_blank"><img src="${shot.path}" alt="${label} screenshot"></a><figcaption>${shot.label || shot.path}</figcaption></figure>`).join('')}</div>` : '<p style="margin-top:12px;color:#888">无截图证据</p>';
...
function renderDualMarkdown(result: any): string {
  const side = (value: any, label: string) => {
    if (!value) return `### ${label}\n\n未执行。\n\n`;
    const shots = screenshotList(value);
    return `### ${label}\n\n- 最终状态：${value.finalState || '-'}\n- 语义路径：${value.semanticPath?.join(' → ') || '-'}\n- 错误：${value.error || value.metadata?.actionError || '-'}\n\n截图：${shots.length ? shots.map((s: any) => `- [${s.label || s.path}](${s.path})`).join('\n') : '无'}\n\n`;
  };
