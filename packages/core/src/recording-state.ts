...
import type { ProcessRecordingStatus } from './models/command-result.js';
...
export function readRecordingState(flowtraceRoot: string): RecordingState | null {
  try {
    const value: unknown = JSON.parse(readFileSync(join(flowtraceRoot, RECORDING_STATE_FILE), 'utf8'));
...
export function writeRecordingState(flowtraceRoot: string, state: RecordingState): void {
  const path = join(flowtraceRoot, RECORDING_STATE_FILE);
  mkdirSync(dirname(path), { recursive: true });
  writeFileSync(path, JSON.stringify(state, null, 2), 'utf8');
}

export function clearRecordingState(flowtraceRoot: string): void {
  try {
...
export function createNotRecordedState(processId: string): RecordingState {
  const now = new Date().toISOString();
  return {
    schemaVersion: 1,
    processId,
...
function isRecordingStatus(value: unknown): value is ProcessRecordingStatus {
