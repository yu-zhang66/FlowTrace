...
function locatorFromExpression(expression: string): string | undefined {
  const testId = expression.match(/getByTestId\(\s*(['"])(.*?)\1\s*\)/);
  if (testId) return `testid=${testId[2]}`;
  const role = expression.match(/getByRole\(\s*(['"])(.*?)\1(?:\s*,\s*\{\s*name:\s*(['"])(.*?)\3\s*\})?\s*\)/);
  if (role) return role[3] ? `role=${role[2]}[name=${role[4]}]` : `role=${role[2]}`;
  const label = expression.match(/getByLabel\(\s*(['"])(.*?)\1\s*\)/);
  if (label) return `label=${label[2]}`;
  const locator = expression.match(/locator\(\s*(['"])(.*?)\1\s*\)/);
  if (locator) return `locator=${locator[2]}`;
  return undefined;
}

function makeStep(line: number, sourceText: string, data: Omit<RecordedStep, 'id' | 'sourceLine' | 'sourceText'>, index: number): RecordedStep {
  return { id: `step-${index + 1}`, sourceLine: line, sourceText, ...data };
}

export function parsePlaywrightRecording(source: string, options: ParseRecordingOptions = {}): RecordingArtifact {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const line = index + 1;
    if (!sourceText || sourceText.startsWith('//') || sourceText.startsWith('import ') || sourceText.startsWith('test(')) continue;
    ...
    if (fill) {
      ...
      if (locator) steps.push(makeStep(line, sourceText, { type: 'fill', locator, value: unquote(fill[3]) }, steps.length));
      else warnings.push(`Unsupported fill locator at line ${line}`);
      continue;
    }
    ...
    if (click) {
      ...
      if (locator) steps.push(makeStep(line, sourceText, { type: 'click', locator }, steps.length));
      else warnings.push(`Unsupported click locator at line ${line}`);
      continue;
    }
    ...
    if (textAssertion) {
      ...
      if (locator) steps.push(makeStep(line, sourceText, { type: 'assertion', locator, assertionType: 'text', expected: textAssertion[3] }, steps.length));
      else warnings.push(`Unsupported text assertion locator at line ${line}`);
      continue;
    }
    ...
    if (valueAssertion) {
      ...
      if (locator) steps.push(makeStep(line, sourceText, { type: 'assertion', locator, assertionType: 'value', expected: valueAssertion[3] }, steps.length));
      else warnings.push(`Unsupported value assertion locator at line ${line}`);
      continue;
    }
    ...
    if (visibleAssertion) {
      ...
      if (locator) steps.push(makeStep(line, sourceText, { type: 'assertion', locator, assertionType: 'visible', expected: 'true' }, steps.length));
      else warnings.push(`Unsupported visibility assertion locator at line ${line}`);
      continue;
    }
...
export async function parsePlaywrightRecordingFile(filePath: string, options: ParseRecordingOptions = {}): Promise<RecordingArtifact> {
