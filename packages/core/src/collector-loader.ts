...
import type { Collector, CollectorConfig, CollectorContext } from '@flowtrace/core';

export interface CollectorLoaderOptions {
  /** 采集器目录 */
...
export class CollectorLoader {
  ...
  async loadFromConfig(
    ...
    options: CollectorLoaderOptions
   ...
   * 检查是否是有效的 Collector
   */
  private isCollector(obj: any): obj is Collector {
    return (
      obj &&
 ...
 * 创建默认采集器加载器
 */
export function createCollectorLoader(collectorsDir: string): CollectorLoader {
 ...
 */
export function loadCollectorConfigs(configPath: string): CollectorConfig[] {
