...
export interface ValidationResult {
  valid: boolean;
  fixturePath: string;
  fixtureType: 'flowchart' | 'testCase' | 'report';
...
export interface AllValidationResults {
  flowchartResults: ValidationResult[];
...
export class GoldenFixtureValidator {
   ...
   */
  validateFlowchartFixture(fixturePath: string): ValidationResult {
    ...
    if (jsonMatch) {
      try {
        ...
        errors.push({
          ...
          message: `Failed to parse JSON section: ${(e as Error).message}`,
          location: { path: fixturePath },
          severity: 'error'
        });
      }
   ...
   */
  validateTestCaseFixture(fixturePath: string): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    try {
      ...
      const testCase = schemaResult.data as TestCase;
      ...
      for (let i = 0; i < testCase.steps.length; i++) {
        ...
        if ('type' in step && step.type === 'ACCOUNT_SWITCH') {
          ...
          const switchErrors = validateAccountSwitchStep(step as AccountSwitchStep);
        ...
        const testStep = step as TestCaseStep;
   ...
   */
  validateReportFixture(fixturePath: string): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
    ...
    if (jsonMatch) {
      try {
        ...
        if (validation.valid) {
          const report = jsonData as VerificationReport;
   ...
   */
  private validateReleaseGateConsistency(
   ...
   */
  validateAll(): AllValidationResults {
    const flowchartResults: ValidationResult[] = [];
    const testCaseResults: ValidationResult[] = [];
 ...
 */
export function createGoldenFixtureValidator(options: ValidatorOptions): GoldenFixtureValidator {
