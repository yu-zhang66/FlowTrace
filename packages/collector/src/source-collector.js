...
export class SourceCollector {
    ...
    config;
    sourceRoot = '';
    extensions = ['.java', '.xml', '.yaml', '.yml', '.json', '.properties'];
    excludeDirs = ['target', 'node_modules', '.git', 'test', 'tests', 'build', 'dist'];
     ...
     * 采集融资流程源码事实
     */
    async collectFinancingSourceFacts(projectPath) {
        const facts = [];
        const timestamp = new Date().toISOString();
        ...
        for (const file of javaFiles) {
            ...
            // 检查文件是否匹配融资流程相关模式
            const isFinancingFile = FINANCING_CLASS_PATTERNS.some(pattern => fileName.includes(pattern.replace(/Impl$/, '').replace(/Enum$/, '').replace(/Controller$/, ''))) || this.isFinancingRelatedPath(file);
            if (isFinancingFile) {
     ...
     * 检查文件路径是否与融资流程相关
     */
    isFinancingRelatedPath(filePath) {
     ...
     * 从融资相关文件提取事实
     */
    extractFinancingFacts(filePath, timestamp, projectPath) {
        const facts = [];
        try {
     ...
     * 创建融资 Controller 事实
     */
    createFinancingControllerFact(fileName, path, content, fullPath, timestamp) {
        // 提取包名
     ...
     * 创建融资 Service 事实
     */
    createFinancingServiceFact(fileName, path, content, fullPath, timestamp) {
        // 提取包名
        const packageMatch = content.match(/package\s+([\w.]+);/);
        const packageName = packageMatch ? packageMatch[1] : '';
        // 提取类名
     ...
     * 创建融资 Enum 事实
     */
    createFinancingEnumFact(fileName, path, content, fullPath, timestamp) {
        // 提取包名
        const packageMatch = content.match(/package\s+([\w.]+);/);
        const packageName = packageMatch ? packageMatch[1] : '';
        // 提取类名
     ...
     * 提取融资流程方法信息
     */
    extractFinancingMethods(content, className) {
        const methods = [];
     ...
     * 生成业务摘要
     */
    generateBusinessSummary(className, methods) {
    ...
    extractFromJavaFile(filePath, timestamp) {
        const facts = [];
        try {
            const content = readFileSync(filePath, 'utf-8');
            const fileName = basename(filePath);
    ...
    extractFromXmlFile(filePath, timestamp) {
        const facts = [];
        try {
            const content = readFileSync(filePath, 'utf-8');
            const fileName = basename(filePath);
    ...
    extractFromYamlFile(filePath, timestamp) {
        const facts = [];
        try {
            const content = readFileSync(filePath, 'utf-8');
            const fileName = basename(filePath);
    ...
    createEvidence(source, line, content, metadata) {
        return {
            source,
            line,
            confidence: 0.8,
    ...
    createApiFact(fileName, path, content, fullPath, timestamp) {
        // 提取 API 路径
    ...
    createServiceFact(fileName, path, content, fullPath, timestamp) {
        return {
            id: generateCollectorId('fact'),
            type: 'service',
            category: 'service',
    ...
    createRepositoryFact(fileName, path, content, fullPath, timestamp) {
        return {
            id: generateCollectorId('fact'),
            type: 'data-access',
    ...
    createEntityFact(fileName, path, content, fullPath, timestamp) {
        // 提取表名
    ...
    createConfigFact(fileName, path, content, fullPath, timestamp) {
        return {
            id: generateCollectorId('fact'),
            type: 'configuration',
            category: 'config',
    ...
    createSpringConfigFact(fileName, path, content, fullPath, timestamp) {
        return {
            id: generateCollectorId('fact'),
            type: 'spring-config',
            category: 'config',
    ...
    createBpmnFact(fileName, path, content, fullPath, timestamp) {
        // 提取流程名称
    ...
    createMapperFact(fileName, path, content, fullPath, timestamp) {
        // 提取 SQL 操作
    ...
    createYamlConfigFact(fileName, path, content, fullPath, timestamp) {
        return {
            id: generateCollectorId('fact'),
            type: 'yaml-config',
            category: 'config',
 ...
 * 创建源码采集器配置
 */
export function createSourceCollectorConfig(name = 'source-collector', options) {
    return {
        name,
        type: 'source-scanner',
        enabled: true,
 ...
 * 创建源码采集器实例
 */
export function createSourceCollector(config) {
    return new SourceCollector(config);
}
/**
 * 检测是否为融资流程相关类
 */
export function isFinancingClassName(className) {
