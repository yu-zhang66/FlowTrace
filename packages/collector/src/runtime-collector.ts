...
/**
 * Runtime Collector 配置
 */
export interface RuntimeCollectorConfig extends CollectorConfig {
  options?: {
    /** 数据库连接配置 */
    dbConnection?: {
      host: string;
    ...
    /** 流程实例表名 */
    processInstanceTable?: string;
    /** 流程任务表名 */
    processTaskTable?: string;
    /** 流程历史表名 */
    processHistoryTable?: string;
    /** 业务数据表名 */
 ...
 * 流程实例事实
 */
export interface ProcessInstanceFact {
  processInstanceId: string;
  businessKey: string;
  status: string;
  startTime: string;
...
export class RuntimeCollector implements Collector {
  ...
  readonly type = 'process-instance' as const;
   ...
   * 创建流程实例事实
   */
  private createProcessInstanceFact(
    instance: ProcessInstanceFact, 
    events: ProcessNodeEvent[],
    timestamp: string
  ): CollectedFact {
    return {
   ...
   * 创建流程节点事件事实
   */
  private createProcessEventFact(event: ProcessNodeEvent, timestamp: string): CollectedFact {
    return {
      id: generateCollectorId('runtime'),
      type: 'process-event',
      category: 'runtime',
   ...
   * 脱敏业务数据
   */
  private maskBusinessData(data: Record<string, unknown>): Record<string, unknown> {
   ...
   * 获取流程实例列表
   */
  private async fetchProcessInstances(): Promise<ProcessInstanceFact[]> {
    ...
    try {
      ...
      return result.map((row: any) => this.mapToProcessInstance(row));
    } catch (error) {
   ...
   * 获取流程事件历史
   */
  private async fetchProcessEvents(processInstanceId: string): Promise<ProcessNodeEvent[]> {
    ...
    try {
      ...
      return result.map((row: any) => this.mapToProcessEvent(row));
    } catch (error) {
   ...
   * 映射数据库行为流程实例
   */
  private mapToProcessInstance(row: any): ProcessInstanceFact {
    return {
   ...
   * 映射数据库行为流程事件
   */
  private mapToProcessEvent(row: any): ProcessNodeEvent {
   ...
   * 映射事件类型
   */
  private mapToEventType(type: string): ProcessEventType {
    const upperType = (type || '').toUpperCase();
    
    const typeMap: Record<string, ProcessEventType> = {
      'START': 'START',
      'INITIATE': 'START',
      'SUBMIT': 'SUBMIT',
      'APPROVE': 'APPROVE',
 ...
 */
export function createRuntimeCollectorConfig(
  name: string = 'runtime-collector',
  options?: RuntimeCollectorConfig['options']
 ...
 */
export function createRuntimeCollector(config: RuntimeCollectorConfig): Collector {
