...
export class ApiCollector {
     ...
     * 从源码采集 API 定义
     */
    async collectFromSource(context) {
        const facts = [];
        const timestamp = new Date().toISOString();
     ...
     * 采集 API 调用日志
     */
    async collectApiLogs(timestamp) {
        const facts = [];
     ...
     * 创建 API 调用事实
     */
    createApiCallFact(log, timestamp) {
        // 脱敏处理
     ...
     * 从数据库获取 API 调用日志
     */
    async fetchApiCallLogs(tableName, timeRange) {
        // 这个方法需要数据库连接
 ...
 * 创建 API Collector 配置
 */
export function createApiCollectorConfig(name = 'api-collector', options) {
    return {
        name,
        type: 'api-scanner',
        enabled: true,
 ...
 * 创建 API Collector 实例
 */
export function createApiCollector(config) {
    return new ApiCollector(config);
}
//# sourceMappingURL=api-collector.js.map