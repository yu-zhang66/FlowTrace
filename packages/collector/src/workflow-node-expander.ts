...
/**
 * WorkflowNodeExpander 配置
 */
export interface WorkflowNodeExpanderOptions {
  /** 流程编码 */
  processCode: string;
  /** 数据库连接配置 */
...
export interface WorkflowStep {
  ...
  stepName: string;
  /** 审批类型 */
  approveType: 'NORMAL' | 'COSIGN' | 'TRANSFER' | 'RETURN';
  /** 顺序号 */
  sequence: number;
  /** 步骤类型 */
  stepType: string;
  /** 会签人数（当 approveType 为 COSIGN 时） */
  cosignCount?: number;
  /** 会签通过方式（当 approveType 为 COSIGN 时） */
  cosignPassType?: 'ALL' | 'ANY' | 'COUNT';
  /** 所需人数（当 cosignPassType 为 COUNT 时） */
  cosignRequiredCount?: number;
  /** 转交规则 */
  transferRule?: string;
  /** 退回规则 */
  returnRule?: string;
...
export interface ExpandedStep {
  ...
  accountRef?: string;
  /** 账号类型 */
  accountType: 'INITIATOR' | 'APPROVER' | 'COUNTER_SIGNER' | 'FINAL_APPROVER';
  /** 操作类型 */
  operation: 'submit' | 'approve' | 'reject' | 'return' | 'transfer';
  /** 是否可执行 */
  executable: boolean;
  /** 来源状态 */
  sourceStatus: 'CONFIRMED' | 'PENDING_CONFIRMATION' | 'UNCONFIRMED';
  /** 证据 */
...
export interface StepEvidence {
  /** 证据类型 */
  type: 'database_schema' | 'api_endpoint' | 'code_snippet' | 'runtime_trace';
  /** 来源类型 */
  sourceType: 'database' | 'source_code' | 'api_documentation' | 'runtime_trace';
  /** 表名（当 sourceType 为 database 时） */
  tableName?: string;
  /** 字段名列表 */
  fieldNames?: string[];
  /** 置信度 */
...
export type WorkflowTestCaseStep = z.infer<typeof WorkflowTestCaseStepSchema>;
...
export class WorkflowNodeExpander {
  private readonly options: WorkflowNodeExpanderOptions;
  private connection: any = null;
  private isConnected: boolean = false;

  constructor(options: WorkflowNodeExpanderOptions) {
   ...
   * 检查数据库连接是否可用
   */
  async checkConnection(): Promise<{ available: boolean; reason?: string }> {
    try {
      await this.connect();
   ...
   * 查询流程基本信息
   */
  private async queryWorkflow(processCode: string): Promise<any[]> {
   ...
   * 查询流程步骤
   */
  private async queryWorkflowSteps(workflowId: string): Promise<any[]> {
    const type = this.options.databaseConnection.type;
    let query: string;
   ...
   * 查询步骤人员配置
   */
  private async queryStepUsers(workflowId: string): Promise<any[]> {
    const type = this.options.databaseConnection.type;
    let query: string;
   ...
   * 构建工作流步骤列表
   */
  private buildWorkflowSteps(stepRows: any[], stepUserRows: any[]): WorkflowStep[] {
    // 按步骤 ID 分组审批人
    const approversByStep = new Map<string, WorkflowApprover[]>();
    ...
    for (const userRow of stepUserRows) {
      ...
      const approver: WorkflowApprover = {
   ...
   * 映射审批类型
   */
  private mapApproveType(type?: string): 'NORMAL' | 'COSIGN' | 'TRANSFER' | 'RETURN' {
    if (!type) return 'NORMAL';
    const upper = type.toUpperCase();
    if (upper === 'COSIGN') return 'COSIGN';
   ...
   * 映射审批人类型
   */
  private mapApproverType(type?: string): 'USER' | 'ROLE' | 'DEPARTMENT' | 'DYNAMIC' {
    if (!type) return 'USER';
    const upper = type.toUpperCase();
    if (upper === 'ROLE') return 'ROLE';
    if (upper === 'DEPARTMENT') return 'DEPARTMENT';
    if (upper === 'DYNAMIC') return 'DYNAMIC';
    return 'USER';
  }
   ...
   * 映射会签通过方式
   */
  private mapCosignPassType(type?: string | null): 'ALL' | 'ANY' | 'COUNT' {
    if (!type) return 'ALL';
    const upper = type.toUpperCase();
   ...
   * 展开步骤为可执行步骤
   */
  expandSteps(workflowConfig: WorkflowConfig): ExpandedStep[] {
   ...
   * 展开会签步骤
   */
  private expandCosignStep(step: WorkflowStep, nextStepId?: string): ExpandedStep[] {
   ...
   * 创建转交流程步骤
   */
  private createTransferStep(step: WorkflowStep, nextStepId?: string): ExpandedStep {
   ...
   * 创建退回步骤
   */
  private createReturnStep(step: WorkflowStep, nextStepId?: string): ExpandedStep {
    const approver = step.approvers.find(a => a.isPrimary) || step.approvers[0];
   ...
   * 创建普通审批步骤
   */
  private createNormalStep(step: WorkflowStep, nextStepId?: string): ExpandedStep {
    const approver = step.approvers.find(a => a.isPrimary) || step.approvers[0];
   ...
   * 创建数据库证据
   */
  private createDatabaseEvidence(
    tableName: string,
    fieldNames: string[],
    confirmed: boolean
  ): StepEvidence[] {
   ...
   */
  generateDynamicPlaceholders(): WorkflowTestCaseStep[] {
    const processCode = this.options.processCode;
    const placeholders: WorkflowTestCaseStep[] = [];
    ...
    for (let i = 1; i <= defaultStepCount; i++) {
      ...
      const step: WorkflowTestCaseStep = {
        stepId: `STEP_${i}`,
        name: `[节点名称待确认]`,
   ...
   * 创建占位符流程配置
   */
  private createPlaceholderWorkflowConfig(
    processCode: string,
    timestamp: string
  ): WorkflowConfig {
   ...
   * 转换为 TestCaseStep 格式
   */
  toTestCaseSteps(expandedSteps: ExpandedStep[]): WorkflowTestCaseStep[] {
    return expandedSteps.map(step => {
      const testStep: WorkflowTestCaseStep = {
        stepId: step.stepId,
        name: step.name,
        ...
        evidence: step.evidence.map(e => {
          if (e.type === 'database_schema') return 'database_query';
          if (e.type === 'api_endpoint') return 'api_request';
 ...
 */
export function createWorkflowNodeExpander(
  options: WorkflowNodeExpanderOptions
