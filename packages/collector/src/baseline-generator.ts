...
export interface CollectionSummary {
  ...
  baselines: {
    source: { exists: boolean; path: string };
    runtime: { exists: boolean; path: string; connected: boolean };
    database: { exists: boolean; path: string; connected: boolean };
    api: { exists: boolean; path: string; connected: boolean };
  };
  currentAdapterMode: string;
  isLegacyShadow: boolean;
  canBeTrustedAsRealBaseline: boolean;
  blockers: string[];
}
...
export class BaselineGenerator {
   ...
   * 生成源码基线
   */
  private generateSourceBaseline(facts: any[]): void {
    const data = {
      version: '1.0',
   ...
   * 生成运行时基线
   */
  private generateRuntimeBaseline(facts: any[], connected: boolean): void {
    const data = {
      version: '1.0',
   ...
   * 生成数据库基线
   */
  private generateDatabaseBaseline(facts: any[], connected: boolean): void {
    const data = {
      version: '1.0',
   ...
   * 生成 API 基线
   */
  private generateApiBaseline(facts: any[], connected: boolean): void {
    const data = {
      version: '1.0',
      type: 'api-baseline',
      metadata: {
  ...
  private generateCollectionSummary(
    status?: { runtimeConnected: boolean; databaseConnected: boolean; apiConnected: boolean },
    currentAdapterMode?: string
  ): void {
    ...
    if (!runtimeConnected) blockers.push('运行时数据未采集 - 需要配置数据库连接');
    if (!databaseConnected) blockers.push('数据库数据未采集 - 需要配置数据库连接');
    if (!apiConnected) blockers.push('API 调用记录未采集 - 需要配置 API 端点');
    if (mode === 'legacy-shadow') blockers.push('Current Adapter 仍为 legacy-shadow - 不能作为真实业务测试');
  ...
  private renderSourceBaselineMarkdown(data: any): string {
    let md = `# 源码基线\n\n`;
  ...
  private renderRuntimeBaselineMarkdown(data: any): string {
    let md = `# 运行时基线\n\n`;
    md += `**项目**: ${data.metadata.projectName}\n`;
    md += `**流程**: ${data.metadata.processId}\n`;
    md += `**采集时间**: ${data.metadata.collectedAt}\n`;
    ...
    const processInstances = data.facts.filter((f: any) => f.type === 'process-instance');
  ...
  private renderDatabaseBaselineMarkdown(data: any): string {
    let md = `# 数据库基线\n\n`;
    md += `**项目**: ${data.metadata.projectName}\n`;
    md += `**流程**: ${data.metadata.processId}\n`;
    md += `**采集时间**: ${data.metadata.collectedAt}\n`;
    ...
    md += `| 表名 | 列数 | 行数 |\n`;
    md += `|------|------|------|\n`;
    for (const fact of data.facts.filter((f: any) => f.type === 'database-table')) {
  ...
  private renderApiBaselineMarkdown(data: any): string {
    let md = `# API 基线\n\n`;
    md += `**项目**: ${data.metadata.projectName}\n`;
    md += `**流程**: ${data.metadata.processId}\n`;
    md += `**采集时间**: ${data.metadata.collectedAt}\n`;
  ...
  private renderCollectionSummaryMarkdown(summary: CollectionSummary): string {
  ...
  private countByCategory(facts: any[]): Record<string, number> {
  ...
  private countProcessInstances(facts: any[]): number {
    return facts.filter(f => f.type === 'process-instance').length;
  }

  private countEvents(facts: any[]): number {
    return facts.filter(f => f.type === 'process-event').length;
  }

  private countByStatus(facts: any[]): Record<string, number> {
    const counts: Record<string, number> = {};
  ...
  private extractTableNames(facts: any[]): string[] {
    return facts
  ...
  private extractEndpoints(facts: any[]): string[] {
    return facts
  ...
  private countMethods(facts: any[]): Record<string, number> {
    const counts: Record<string, number> = {};
 ...
 * 创建基线生成器
 */
export function createBaselineGenerator(config: BaselineConfig): BaselineGenerator {
