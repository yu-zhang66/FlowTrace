...
export class BusinessReportGenerator {
     ...
     * 生成完整报告
     */
    generate(scenarios, results, collectionStatus) {
     ...
     * 构建场景详情
     */
    buildScenarioDetails(scenarios, results) {
    ...
    extractNodeDetails(result) {
        if (!result.auditRecords)
            return [];
        return result.auditRecords.map((record) => ({
    ...
    extractApiCalls(result) {
        if (!result.externalCalls)
            return [];
        return result.externalCalls.map((call) => ({
     ...
     * 生成总览页
     */
    generateOverviewPage(collectionStatus) {
        ...
        const p0Count = details.reduce((sum, d) => sum + d.comparison.differences.filter(diff => diff.severity === 'P0').length, 0);
        const p1Count = details.reduce((sum, d) => sum + d.comparison.differences.filter(diff => diff.severity === 'P1').length, 0);
        const p2Count = details.reduce((sum, d) => sum + d.comparison.differences.filter(diff => diff.severity === 'P2').length, 0);
        const p3Count = details.reduce((sum, d) => sum + d.comparison.differences.filter(diff => diff.severity === 'P3').length, 0);
...
<body>
  <div class="container">
    ...
    generateScenarioDetailHtml(detail) {
...
<body>
  ...
  <script>mermaid.initialize({ startOnLoad: true });</script>
     ...
     * 生成差异分析页
     */
    generateDifferenceAnalysisPage() {
...
<body>
  <div class="container">
     ...
     * 生成采集可信度页
     */
    generateCollectionCredibilityPage(collectionStatus) {
        const html = `<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8">
...
<body>
  <div class="container">
    ...
    getPreState(scenario, stepIndex) {
        if (stepIndex === 0)
            return 'DRAFT';
        return 'PROCESSING';
    }
    getPostState(scenario, stepIndex) {
 ...
 * 创建业务报告生成器
 */
export function createBusinessReportGenerator(config) {
    return new BusinessReportGenerator(config || {});
}
//# sourceMappingURL=business-report.js.map