...
export class ScenarioRenderer {
     ...
     * 保存渲染结果
     */
    saveRenderedScenarios(scenarios) {
...
${scenarios.map(s => {
    ...
    renderPreconditionsHtml(scenario) {
        const preconditions = scenario.precondition || {};
        if (Object.keys(preconditions).length === 0) {
    ...
    renderExpectedResultHtml(scenario) {
        const expected = scenario.expected;
        if (!expected)
            return '<p class="result pending">未定义预期结果</p>';
    ...
    renderExpectedDataChanges(scenario) {
        const expected = scenario.expected;
    ...
    renderExpectedDataChangesHtml(scenario) {
        const expected = scenario.expected;
        if (!expected?.database || Object.keys(expected.database).length === 0) {
    ...
    renderExpectedApiCalls(scenario) {
        const expected = scenario.expected;
    ...
    renderExpectedApiCallsHtml(scenario) {
        const expected = scenario.expected;
        if (!expected?.externalCalls || expected.externalCalls.length === 0) {
    ...
    getStepResult(action, scenario) {
        ...
        if (type === 'RETURN')
            return '申请被退回，流程暂停';
        if (type === 'TRANSFER' && scenario.expected?.finalState === 'TRANSFERRED')
            return '放款完成';
 ...
 * 创建场景渲染器
 */
export function createScenarioRenderer(config) {
    return new ScenarioRenderer(config);
}
//# sourceMappingURL=scenario-renderer.js.map