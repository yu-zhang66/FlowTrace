...
import type { TestCase, TestCaseStep, AccountSwitchStep } from '@flowtrace/core';
import type { FlowchartDocument } from '@flowtrace/core';
import { z } from 'zod';
...
const CASE_TYPE_LABELS_ZH: Record<string, string> = {
...
const ACCOUNT_TYPE_LABELS_ZH: Record<string, string> = {
  'INITIATOR': '发起人',
  'APPROVER': '审批人',
  'COUNTER_SIGNER': '会签人',
...
const OPERATION_LABELS_ZH: Record<string, string> = {
  'view': '查看',
  'create': '创建',
  'update': '更新',
  'delete': '删除',
...
const SOURCE_STATUS_LABELS_ZH: Record<string, string> = {
  'AUTO_GENERATED': '自动生成',
...
const WORKFLOW_STATUS_LABELS_ZH: Record<string, string> = {
  'not_reached': '未到达',
  'in_progress': '进行中',
  'completed': '已完成',
  'skipped': '已跳过',
...
const FLOW_STATUS_LABELS_ZH: Record<string, string> = {
  'pending': '待处理',
  'approved': '已通过',
  'rejected': '已拒绝',
  'returned': '已退回',
...
const POST_FLOW_STATUS_LABELS_ZH: Record<string, string> = {
  'completed': '已完成',
  'terminated': '已终止',
  'archived': '已归档'
};
...
export interface TestCaseMarkdownRendererOptions {
  language?: 'zh' | 'en';
  includeMermaid?: boolean;
  includeAccountSwitchDetail?: boolean;
}

const DEFAULT_OPTIONS: TestCaseMarkdownRendererOptions = {
  language: 'zh',
  includeMermaid: true,
...
export class TestCaseMarkdownRenderer {
  private options: TestCaseMarkdownRendererOptions;
  private labels: {
    ...
    postFlowStatus: Record<string, string>;
  };

  constructor(options: TestCaseMarkdownRendererOptions = {}) {
   ...
   * 渲染单个测试案例
   */
  render(testCase: TestCase): string {
   ...
   * 渲染所有测试案例
   */
  renderAll(testCases: TestCase[]): string {
    let md = '# 测试案例集\n\n';
   ...
   * 渲染配置检查结果
   */
  renderConfigCheck(result: ConfigCheckResult): string {
   ...
   * 渲染案例索引
   */
  renderIndex(testCases: TestCase[]): string {
    let md = '# 测试案例索引\n\n';
   ...
   * 渲染 Mermaid 流程图
   */
  renderMermaid(flowchart: FlowchartDocument): string {
  ...
  private validateTestCase(data: unknown): { valid: boolean; errors?: string[] } {
    try {
  ...
  private validateFlowchart(data: unknown): { valid: boolean; errors?: string[] } {
    try {
  ...
  private renderHeader(tc: TestCase): string {
...
`;
  ...
  private renderBasicInfo(tc: TestCase): string {
    let md = '## 基本信息\n\n';
  ...
  private renderPreconditions(tc: TestCase): string {
    let md = '## 前置条件\n\n';
  ...
  private renderRequiredResources(tc: TestCase): string {
    let md = '## 所需资源\n\n';
  ...
  private renderSteps(tc: TestCase): string {
    let md = '## 测试步骤\n\n';
    ...
    for (let i = 0; i < tc.steps.length; i++) {
      ...
      if ('type' in step && step.type === 'ACCOUNT_SWITCH') {
        md += this.renderAccountSwitchStep(step as AccountSwitchStep, i + 1);
      } else {
        md += this.renderTestStep(step as TestCaseStep, i + 1);
      }
    }
  ...
  private renderTestStep(step: TestCaseStep, index: number): string {
  ...
  private renderAccountSwitchStep(step: AccountSwitchStep, index: number): string {
  ...
  private renderExpectedResults(tc: TestCase): string {
    let md = '## 预期结果\n\n';
  ...
  private renderAccountSwitchTimeline(tc: TestCase): string {
    const accountSwitchSteps = tc.steps.filter(
      s => 'type' in s && s.type === 'ACCOUNT_SWITCH'
    ) as AccountSwitchStep[];
  ...
  private renderDataCleanup(tc: TestCase): string {
    let md = '## 数据清理策略\n\n';
  ...
  private renderFooter(tc: TestCase): string {
    let md = '---\n\n';
  ...
  private calculateStats(testCases: TestCase[]): {
    total: number;
    executable: number;
    ...
    for (const tc of testCases) {
      if (tc.executable) stats.executable++;
      if (tc.sourceStatus === 'PENDING_CONFIRMATION') stats.pendingConfirmation++;
      if (tc.severity === 'P0') stats.p0++;
      if (tc.severity === 'P1') stats.p1++;
...
export function createTestCaseMarkdownRenderer(
  options?: TestCaseMarkdownRendererOptions
): TestCaseMarkdownRenderer {
