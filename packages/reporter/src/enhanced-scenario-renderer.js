...
function getLanguageText(zh, en) {
    return zh;
}
// ============================================================
// Mermaid Flowchart Generator
// ============================================================
function generateMermaidFlowchart(scenario) {
    const nodes = [];
    const transitions = [];
...
// ============================================================
function generateMermaidSequenceDiagram(scenario) {
    const participants = [];
    const interactions = [];
...
function renderLegacyActions(scenario) {
    const actions = scenario.legacyActions;
...
function renderExpectedResults(scenario) {
    ...
    // Final state
    const finalState = final?.finalState || expected?.finalState || '-';
    sections.push('');
...
function renderGenerationMetadata(meta) {
    if (!meta)
        return '';
    const lines = [];
    lines.push('## 十三、生成元数据');
...
function renderMarkdown(scenario, config) {
...
// HTML Renderer
// ============================================================
function renderHtml(scenario, markdown, config) {
    const mermaidFlowchart = generateMermaidFlowchart(scenario);
    const mermaidSequence = generateMermaidSequenceDiagram(scenario);
    return `<!DOCTYPE html>
<html lang="zh-CN">
...
<body>
  <div class="container">
    ...
    <table>
      ...
      <tbody>
        ${Object.entries(scenario.actors).map(([key, actor]) => `<tr><td>${key}</td><td>${actor.role}</td><td>${actor.usernameRef}</td><td>${actor.description || '-'}</td></tr>`).join('')}
      </tbody>
    </table>
    ` : ''}
    ...
    <table>
      ...
      <tbody>
        ${Object.entries(scenario.testData).map(([key, value]) => `<tr><td>${key}</td><td>${String(value)}</td></tr>`).join('')}
      </tbody>
    </table>
    ` : ''}
...
//# sourceMappingURL=enhanced-scenario-renderer.js.map