...
export class DifferenceClassifier {
    static BLOCKING_SEVERITIES = ['P0', 'P1'];
    static classify(difference) {
    ...
    static sortBySeverity(a, b) {
        const order = { 'P0': 0, 'P1': 1, 'P2': 2, 'P3': 3 };
        return (order[a] ?? 999) - (order[b] ?? 999);
    }
    static groupBySeverity(differences) {
        const grouped = {
            P0: [],
            P1: [],
            P2: [],
...
export class ReportGenerator {
    ...
    generateVerificationRun(projectId, results, failOn = ['P0', 'P1']) {
    ...
    generateMarkdown(run, projectName) {
