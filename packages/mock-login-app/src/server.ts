...
function createApp(config: AppConfig): express.Application {
  ...
  app.post('/login', async (req: Request, res: Response) => {
    ...
    const users: MockUser[] = req.app.locals.users;
...
function getLoginPageHtml(error: string | undefined, mode: 'legacy' | 'current'): string {
  let errorMessage = '';
  let errorClass = '';
...
function getDashboardHtml(user: MockUser, mode: 'legacy' | 'current'): string {
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const mode = (args.includes('--current') ? 'current' : 'legacy') as 'legacy' | 'current';
...
export { createApp, startServer, type AppConfig };
