...
describe('runtime/redaction', () => {
  ...
  it('redacts nested object fields case-insensitively', () => {
    ...
    expect((out.nested as Record<string, unknown>).Authorization).toBe(REDACT_MARKER);
  });

  it('redacts arrays element-by-element without losing order', () => {
    const out = redactValue([{ token: 't1' }, { token: 't2' }, { safe: 1 }]) as Array<Record<string, unknown>>;
  ...
  it('caps recursion depth to prevent stack overflow', () => {
    const deep: Record<string, unknown> = { a: { b: { c: { d: { e: { password: 'leak' } } } } } };
    const out = redactValue(deep, { maxDepth: 2 }) as Record<string, unknown>;
    const walk = (v: unknown, depth: number): unknown => {
      if (depth > 6) return '…';
      if (v && typeof v === 'object') {
        const r: Record<string, unknown> = {};
        for (const [k, val] of Object.entries(v as Record<string, unknown>)) r[k] = walk(val, depth + 1);
        return r;
      }
      return v;
    };
  ...
  it('honours per-system custom field overrides', () => {
    ...
    expect((out as Record<string, unknown>).tenantSecret).toBe(REDACT_MARKER);
    expect((out as Record<string, unknown>).safe).toBe('ok');
  });
  ...
  it('redactHeaders redacts the default header set and respects overrides', () => {
    const out = redactHeaders({
      ...
      'X-Tenant': 'acme',
    }, { headers: ['x-tenant'] }) as Record<string, unknown>;
  ...
  it('createRedactor returns a function with redactHeaders attached', () => {
    ...
    expect(typeof (fn as any).redactHeaders).toBe('function');
  });
