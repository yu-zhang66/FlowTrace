 ...
 * Flow Adapter 接口实现
 */
export class CurrentFlowAdapter {
    name;
    type = 'current';
    context;
    options;
    ...
    constructor(context, options = {}) {
        ...
        this.options = { mode: 'legacy-shadow', ...options };
    }
    async initialize() {
    ...
    executeLegacyShadow(action, startTime, actionData) {
    ...
    handleApprove(action, startTime, data) {
        switch (this.state) {
            case 'SUBMITTED':
    ...
    addAuditRecord(action, actor, timestamp) {
    ...
    createErrorResult(action, startTime, error) {
        return {
            scenarioId: action.type,
            adapter: 'current',
            actions: [action],
    ...
    addSemanticPath(path) {
        this.semanticPath.push(path);
    }
    addSimpleExternalCall(call) {
...
class ApiHandler {
    ...
    async executeAction(action, startTime, data, adapter) {
        // API 端点映射
        const endpointMap = {
    ...
    createSimpleExternalCall(endpoint, method, request, response) {
        return {
            endpoint,
            method,
            request,
...
class DatabaseHandler {
    ...
    async executeDbOperation(table, operation, data, adapter) {
        // 这是一个占位实现
    ...
    async executeAction(action, startTime, data, adapter) {
        ...
        try {
            ...
            const pageState = await this.page.evaluate(() => {
                ...
                const stateEl = globalThis.document?.querySelector('[data-state]');
                return {
 ...
 * 创建 Current Adapter
 */
export function createCurrentAdapter(context, options) {
