...
export const DslRequestStepSchema = StepBase.extend({
    ...
    system: z.string().optional(),
}).refine((v) => Boolean(v.endpoint) || Boolean(v.path) || Boolean(v.url), { message: 'request step requires `endpoint`, `path`, or `url`' });
...
export const DslStepSchema = z.union([
    DslGotoStepSchema,
    DslFillStepSchema,
    DslClickStepSchema,
    DslSelectStepSchema,
...
export const RuntimeConfigSchema = z
    ...
    systems: z.record(RuntimeSystemConfigSchema).refine((m) => Object.keys(m).length > 0, { message: 'runtime.systems must contain at least one system entry' }),
})
    .refine((v) => v.adapter !== 'external' || Boolean(v.external), { message: 'runtime.external is required when runtime.adapter is `external`' });
...
export function validateProcessDsl(input) {
 ...
 */
export function issuesFromZodError(err) {
