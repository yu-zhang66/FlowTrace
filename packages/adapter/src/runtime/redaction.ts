...
export const DEFAULT_REDACT_FIELDS: ReadonlyArray<string> = Object.freeze([
  'password',
  'pwd',
  'pass',
  'passwd',
...
export const DEFAULT_REDACT_HEADERS: ReadonlyArray<string> = Object.freeze([
  'authorization',
  'proxy-authorization',
  'cookie',
  'set-cookie',
...
export interface RedactOptions {
  fields?: string[];
  headers?: string[];
  /** Maximum depth before values are summarised. */
  maxDepth?: number;
}
...
export interface Redactor {
  ...
  redactHeaders(headers: unknown): unknown;
}
...
export function createRedactor(options: RedactOptions = {}): Redactor {
  const fieldSet = new Set<string>(FROZEN_DEFAULT_FIELDS);
  for (const f of options.fields ?? []) fieldSet.add(String(f).toLowerCase());

  const headerSet = new Set<string>(FROZEN_DEFAULT_HEADERS);
  for (const h of options.headers ?? []) headerSet.add(String(h).toLowerCase());

  const maxDepth = options.maxDepth ?? 8;

  const redactKey = (key: string): boolean => fieldSet.has(key.toLowerCase());
  const redactHeader = (key: string): boolean => headerSet.has(key.toLowerCase());

  const walk = (value: unknown, depth: number): unknown => {
    if (depth > maxDepth) return REDACT_MARKER;
  ...
  const top = ((value: unknown) => walk(value, 0)) as Redactor;
 ...
 */
export function redactHeaders(headers: unknown, options: RedactOptions = {}): unknown {
