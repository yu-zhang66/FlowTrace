...
export class PlaywrightBrowserTestAdapter {
    name;
    type;
    config;
    browser = null;
    ...
    retiredContexts = new Set();
    evidenceDir;
    constructor(config, evidenceDir = '.flowtrace/evidence') {
    ...
    async initialize() {
        ...
        const launchWithRetry = async (options, label) => {
            let lastError;
    ...
    attachPageListeners(page) {
        page.on('console', msg => {
            if (msg.type() === 'error')
                console.log(`[${this.type} Console Error] ${msg.text()}`);
        });
        page.on('pageerror', error => console.log(`[${this.type} Page Error] ${error.message}`));
        page.on('requestfailed', request => {
            const failure = request.failure();
            if (failure)
                console.log(`[${this.type} Network Failed] ${request.url()} - ${failure.errorText}`);
        });
    }
    async login(actor, input) {
        ...
        const finalState = semanticPath.includes('AUTHENTICATED') ? 'AUTHENTICATED' : 'LOGIN_FAILED';
        return {
            finalState,
            semanticPath,
            currentUrl,
     ...
     */
    async handleCaptcha(evidence, semanticPath) {
        if (!this.page)
        ...
        for (let attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                // 3. 填写验证码
                await this.waitAndFill(this.config.captchaInputSelector || 'input[placeholder="请输入验证码"]', captchaText);
    ...
    inferErrorType(errorHint, pageTitle) {
        if (!errorHint && !pageTitle)
            return 'UNKNOWN';
    ...
    async captureEvidence(evidence, filename) {
 ...
 */
export function createPlaywrightBrowserAdapter(config, evidenceDir) {
    return new PlaywrightBrowserTestAdapter(config, evidenceDir);
}
//# sourceMappingURL=playwright-browser-adapter.js.map