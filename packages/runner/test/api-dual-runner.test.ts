...
} from '@flowtrace/runner';

describe('ApiDualRunner', () => {
  let runner: ApiDualRunner;

  const createTestMapping = (): ApiMapping => ({
    id: 'testMapping',
  ...
  describe('request mapping', () => {
    it('should map request fields according to mapping', async () => {
      ...
      try {
        await runnerWithMapping.executeDualRun(
          'testMapping',
          'http://legacy.local',
          'http://current.local',
          { legacyField: 'value1', amount: '100' }
        );
...
describe('AuthConfig types', () => {
  ...
  it('should support API key auth', () => {
    const auth = { type: 'api-key' as const, apiKey: 'test-api-key' };
