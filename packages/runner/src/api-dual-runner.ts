...
import type { ScenarioAction, ExecutionResult } from '@flowtrace/core';
...
export interface ApiMapping {
  ...
  /** 请求字段映射 */
  requestMapping: ApiFieldMapping[];
  /** 响应字段映射 */
...
export interface ApiFieldMapping {
  from: string;
  to: string;
  transform?: 'string' | 'number' | 'boolean' | 'date' | 'none';
...
export interface ApiComparisonResult {
  ...
  technicalDiffs: {
    httpDiff: boolean;
    fieldNameDiff: boolean;
    fieldStructureDiff: boolean;
    statusCodeDiff: boolean;
    errorCodeDiff: boolean;
  };
}

export interface ApiDifference {
  category: 'http' | 'field_name' | 'field_structure' | 'field_value' | 'status_code' | 'error_code' | 'business_semantic';
  field?: string;
  legacy?: unknown;
  current?: unknown;
  description: string;
...
export class ApiDualRunner {
  ...
  private legacyAuth?: AuthConfig;
  private currentAuth?: AuthConfig;
  private defaultTimeout: number = 30000;

  constructor(options?: {
    mappings?: ApiMapping[];
    legacyAuth?: AuthConfig;
   ...
   * 批量添加 API 映射
   */
  addMappings(mappings: ApiMapping[]): void {
   ...
   * 执行 API 双跑
   */
  async executeDualRun(
    actionType: string,
    legacyBaseUrl: string,
    currentBaseUrl: string,
    requestData: Record<string, unknown>
  ): Promise<{
    legacy: ApiCallResult;
    current: ApiCallResult;
   ...
   * 查找 API 映射
   */
  private findMapping(actionType: string): ApiMapping | undefined {
    // 根据 action type 查找映射
   ...
   * 映射请求数据
   */
  private mapRequest(mapping: ApiMapping, data: Record<string, unknown>): Record<string, unknown> {
  ...
  private async callLegacyApi(
    ...
    try {
      const response = await this.fetchWithTimeout(
        url,
        mapping.legacy.method,
        data,
        this.legacyAuth,
        mapping.timeout || this.defaultTimeout
      );
   ...
   * 标准化响应数据
   */
  private normalizeResponse(mapping: ApiMapping, body: unknown): Record<string, unknown> {
  ...
  private compareResults(
    ...
    // 业务语义判断
    const semanticDiff = differences.some(d => 
      d.severity === 'P0' || d.severity === 'P1'
    );
 ...
 * 创建标准 API 映射
 */
export function createStandardMappings(): ApiMapping[] {
  return [
    {
      id: 'submitFinancing',
