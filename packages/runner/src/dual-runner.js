...
export class DualRunner {
    ...
    currentAdapter;
    config;
    constructor(legacyAdapter, currentAdapter, config = {}) {
    ...
    async runScenario(scenario) {
        ...
        let error;
        try {
            legacyResult = await this.executeWithTimeout(this.legacyAdapter.executeScenario(scenario));
        }
        catch (err) {
        ...
        try {
            currentResult = await this.executeWithTimeout(this.currentAdapter.executeScenario(scenario));
        }
        catch (err) {
            error = error
    ...
    compareResults(scenario, legacyResult, currentResult) {
        const differences = [];
    ...
    async executeWithTimeout(promise) {
        return Promise.race([
            promise,
            new Promise((_, reject) => setTimeout(() => reject(new Error('Execution timeout')), this.config.timeout || 30000))
        ]);
    }
}
//# sourceMappingURL=dual-runner.js.map