...
// ============================================================
function buildSemanticPath(steps, results) {
    const nodes = [];
...
export class EnhancedDualRunner {
    ...
    async execute(scenario) {
        ...
        const semanticComparison = this.compareSemanticPaths(scenario, legacySemanticPath, currentSemanticPath);
     ...
     */
    async executeLegacy(scenario, stepResults) {
        ...
        for (const step of (scenario.steps || [])) {
            ...
            if (step.browser?.legacy && this.browserExecutor) {
                try {
                    const browserResult = await this.browserExecutor.executeStep({ ...step.browser, legacy: step.browser.legacy }, 'legacy');
     ...
     */
    async executeCurrent(scenario, stepResults) {
        const browserResults = [];
        const apiResults = [];
        // Execute each step
        for (const step of (scenario.steps || [])) {
            ...
            if (step.browser?.current && this.browserExecutor) {
                try {
                    const browserResult = await this.browserExecutor.executeStep({ ...step.browser, current: step.browser.current }, 'current');
                    result.browserResult = {
                        stepId: step.id,
     ...
     */
    determineFinalState(scenario, stepResults, side) {
     ...
     * Compare semantic paths
     */
    compareSemanticPaths(scenario, legacyPath, currentPath) {
     ...
     */
    compareEventPaths(legacy, current) {
     ...
     */
    collectDifferences(scenario, stepResults, semanticComparison) {
        const differences = [];
...
//# sourceMappingURL=enhanced-dual-runner.js.map